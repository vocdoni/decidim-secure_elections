      ...
      describe CensusCsv do
        ...
        describe CensusCsv::Reader do
          ...
          describe "#people_count" do
            context "when the file is the downloaded template, untouched" do
              let(:content) { "Member number;First name;Email\n000123;Ada;ada@example.org\n" }

              it "counts nobody, because the example line is not a person" do
                reader.load!

                expect(reader.row_count).to eq(1)
                expect(reader.example_row_count).to eq(1)
                expect(reader.people_count).to eq(0)
              end
            end

            context "when real people were added under the example line" do
              let(:content) { "Member number;First name;Email\n000123;Ada;ada@example.org\n7;Rosalind;rosalind@example.org\n" }

              it "counts only the real people" do
                reader.load!

                expect(reader.people_count).to eq(1)
              end
            end
          end
        ...
        describe CensusCsv::Fields do
          ...
          describe ".example_row?" do
            it "recognises the template's own line, whatever the column order" do
              expect(described_class.example_row?(["000123", "Ada", "ada@example.org"])).to be(true)
              expect(described_class.example_row?(["ada@example.org", " Ada "])).to be(true)
            end

            it "does not recognise a real person" do
              expect(described_class.example_row?(["000123", "Rosalind", "rosalind@example.org"])).to be(false)
            end

            it "does not recognise an empty line" do
              expect(described_class.example_row?([nil, "  "])).to be(false)
            end
          end
